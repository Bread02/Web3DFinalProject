// JavaScript Gallery Generator

// Further modified from lab code to split the three different models into different categories.
// Each gallery page will display the Cup, or Bottle, or Can.

$(document).ready(function () {
	// Create the XMLHttpRequest Object for communicating with the web server
	var xmlHttp = new XMLHttpRequest();
	// Stores newly generated gallery HTML code
	var htmlCodeCup = "";
	// Temporarily stores server response while code is generated
	var response;
	// Set up a path variable to the PHP function that reads the image directory to find the thumbnail file names
	var send = "application/php/hookCup.php";
	// Open the connection to the web server
	xmlHttp.open("GET", send, true);
	// Tell the server that the client has no outgoing message, i.e. we are sending nothing to the server
	xmlHttp.send(null);
	// Check we get a valid server response
	xmlHttp.onreadystatechange = function () {
		if (xmlHttp.readyState == 4) {
			// Response handler code
			//alert(xmlHttp.responseText);
			response = xmlHttp.responseText.split("~");
			// Loop round the response array
			for (var i = 0; i < response.length; i++) {
				// Handeler to build the HTML string
				// Use this to provide a link to the image

				// Cup Gallery
				htmlCodeCup += '<a href="assets/images/gallery_images/cup_gallery' + response[i] + ' " ' + ' data-fancybox data-caption="Rendering of Costa Cup in Blender" > ';
				htmlCodeCup += '<img class="card-img-top img-thumbnail" src="assets/images/gallery_images/cup_gallery' + response[i] + '"/>';
				htmlCodeCup += '</a>';
			}
			// Return the HTML string to each of the 3 3D App element blocks, coke, sprite and pepper galleries
			document.getElementById('gallery_cup').innerHTML = htmlCodeCup;
		}
	}

	// Create the XMLHttpRequest Object for communicating with the web server
	var xmlHttp2 = new XMLHttpRequest();
	// Stores newly generated gallery HTML code
	var htmlCodeBottle = "";
	// Temporarily stores server response while code is generated
	var response;
	// Set up a path variable to the PHP function that reads the image directory to find the thumbnail file names
	var send = "application/php/hookBottle.php";
	// Open the connection to the web server
	xmlHttp2.open("GET", send, true);
	// Tell the server that the client has no outgoing message, i.e. we are sending nothing to the server
	xmlHttp2.send(null);

	xmlHttp2.onreadystatechange = function () {
		if (xmlHttp2.readyState == 4) {
			// Response handler code
			//alert(xmlHttp.responseText);
			response = xmlHttp2.responseText.split("~");
			// Loop round the response array

			// do the above again but for bottle
			for (var i = 0; i < response.length; i++) {
				// Handeler to build the HTML string
				// Use this to provide a link to the image

				// Cup Gallery
				htmlCodeBottle += '<a href="assets/images/gallery_images/bottle_gallery' + response[i] + ' " ' + ' data-fancybox data-caption="Rendering of Costa Bottles in Blender" > ';
				htmlCodeBottle += '<img class="card-img-top img-thumbnail" src="assets/images/gallery_images/bottle_gallery' + response[i] + '"/>';
				htmlCodeBottle += '</a>';
			}
			// Return the HTML string to each of the 3 3D App element blocks, coke, sprite and pepper galleries
			document.getElementById('gallery_bottle').innerHTML = htmlCodeBottle;
		}
	}

	// Create the XMLHttpRequest Object for communicating with the web server
	var xmlHttp3 = new XMLHttpRequest();
	// Stores newly generated gallery HTML code
	var htmlCodeCan = "";
	// Temporarily stores server response while code is generated
	var response;
	// Set up a path variable to the PHP function that reads the image directory to find the thumbnail file names
	var send = "application/php/hookCan.php";
	// Open the connection to the web server
	xmlHttp3.open("GET", send, true);
	// Tell the server that the client has no outgoing message, i.e. we are sending nothing to the server
	xmlHttp3.send(null);

	xmlHttp3.onreadystatechange = function () {
		if (xmlHttp3.readyState == 4) {
			// Response handler code
			//alert(xmlHttp.responseText);
			response = xmlHttp3.responseText.split("~");
			// Loop round the response array


			// do the above again but for can
			// do the above again but for bottle
			for (var i = 0; i < response.length; i++) {
				// Handeler to build the HTML string
				// Use this to provide a link to the image

				// Cup Gallery
				htmlCodeCan += '<a href="assets/images/gallery_images/can_gallery' + response[i] + ' " ' + ' data-fancybox data-caption="Rendering of Costa Cans in Blender" > ';
				htmlCodeCan += '<img class="card-img-top img-thumbnail" src="assets/images/gallery_images/can_gallery' + response[i] + '"/>';
				htmlCodeCan += '</a>';
			}
			// Return the HTML string to each of the 3 3D App element blocks, coke, sprite and pepper galleries
			document.getElementById('gallery_can').innerHTML = htmlCodeCan;
		}

	}
});